...
#include "interrupt.h"
#include "clock.h"
int32_t Motor_L_count=0;
int32_t Motor_R_count=0;
...
{
    uint32_t Left_A=DL_GPIO_getEnabledInterruptStatus(GPIO_ECONDER_PORT,GPIO_ECONDER_LEFT_A_PIN);//左轮中断标志位获取
    uint32_t Right_A=DL_GPIO_getEnabledInterruptStatus(GPIO_ECONDER_PORT,GPIO_ECONDER_RIGHT_A_PIN);//右轮中断标志位获取
    //计数编码器数值
    if ((Left_A & GPIO_ECONDER_LEFT_A_PIN))
    {
        if(DL_GPIO_readPins(GPIO_ECONDER_PORT,GPIO_ECONDER_LEFT_B_PIN))
        {
        Motor_L_count++;
        }
        else
        ...
        DL_GPIO_clearInterruptStatus(GPIO_ECONDER_PORT, GPIO_ECONDER_LEFT_A_PIN);
    }
    //计数编码器数值
    if ((Right_A & GPIO_ECONDER_RIGHT_A_PIN))
    {
        if(DL_GPIO_readPins(GPIO_ECONDER_PORT,GPIO_ECONDER_RIGHT_B_PIN))
        {
             Motor_R_count++;
        }
        else
        ...
        DL_GPIO_clearInterruptStatus(GPIO_ECONDER_PORT,GPIO_ECONDER_RIGHT_A_PIN);
    }
